...
const SideMenu: React.FC<SideMenuProps> = ({ isOpen, onClose, onNavigate }) => {
  return (
    <>
      {/* Backdrop - Absolute to stay within the mobile frame container */}
      <div 
        className={`absolute inset-0 bg-black/70 backdrop-blur-[2px] z-[60] transition-opacity duration-500 ${isOpen ? 'opacity-100' : 'opacity-0 pointer-events-none'}`}
        onClick={onClose}
      />
      
      {/* Side Panel - Absolute to stay within the mobile frame container */}
      <div 
        className={`absolute inset-y-0 left-0 w-[80%] max-w-[300px] bg-[#1c1c1c] z-[70] transform transition-transform duration-500 cubic-bezier(0.16, 1, 0.3, 1) border-r border-white/10 shadow-[20px_0_50px_rgba(0,0,0,0.5)] ${isOpen ? 'translate-x-0' : '-translate-x-full'}`}
      >
        {/* Header Decor */}
        <div className="h-40 bg-[#d48c31] relative flex items-end px-8 pb-8 overflow-hidden">
          <div 
          ...
          <div className="relative z-10">
            <h3 className="header-font text-[28px] text-zinc-900 leading-tight">Al Ahmadi</h3>
            <p className="text-[10px] text-zinc-900/70 font-black tracking-[0.4em] uppercase mt-1">Heritage Kitchen</p>
          </div>
        ...
        <div className="py-8 px-4 flex flex-col gap-1">
          {[
            ...
            { id: 'menu', label: 'Full Menu', icon: 'M4 6h16M4 12h16M4 18h16' },
            ...
            >
              <div className="w-8 h-8 rounded-xl bg-zinc-800/50 flex items-center justify-center group-hover:bg-[#d48c31]/10 transition-colors">
        ...
        <div className="mt-auto px-10 pb-10">
          <div className="h-[1px] w-10 bg-zinc-800 mb-8"></div>
          <div className="space-y-1">
            ...
            <p className="text-[9px] text-[#d48c31]/40 font-black uppercase tracking-[0.2em] pt-4">© 2026</p>
          </div>
        </div>
      </div>
    </>
